"""The meta-scan: every AST/grep-guard helper this suite carries has been shown
to catch something, by a test that *calls it*.

*A scan that has never fired has not been shown to check anything* is the rule
this repo's own tests already state in their docstrings (`tests/test_cards.py`,
`tests/test_corpus_counts.py`, `tests/test_seed_dirs_in_step.py` and
`tests/test_vendor_pins.py` each plant a violation for the scan they carry).
This file is that rule turned into a test that reads the *other* test files: it
...
**Ported, and re-grounded.** The discovery rule and the plant rule below are
homestead-affairs/homestead-ledger's `tests/test_scans_fire.py` (public),
copied for their shape on 2026-09-12 and verified here against this repo's own
test files rather than re-derived, because a meta-scan that disagreed between
repos about what counts as "planted" would be exactly the drift this sweep
exists to close. The real-file checks are this repo's; the fixture modules the
plant tests write name this repo's helpers.

What the port found here, measured before anything was planted: six
module-level scan helpers in five files that no plant test called —
`_strings` and `_roles_from_source` (`test_cards.py`), `_count_raw_entries`
(`test_corpus_counts.py`), `_names_a_non_suppressed_credential`
(`test_release_wiring.py`), `_literal_hosts` (`test_source_hosts.py`) and
`_assert_one_bucket_each` (`test_sources.py`) — all planted in the same commit
as this file; and eleven tests whose *own body* is a scan, two of which
(`test_corpus_counts.py`) were factored into planted helpers here and nine of
which are recorded in `INLINE_SCANS_STILL_TO_FACTOR` below as a ratchet.
...
* it reads a file's text (`.read_text()`/`.read_bytes()`) *and* asks a
  ...
  with no regex in it, which `tests/test_corpus_counts.py::_count_raw_entries`
  is, **or**
* it walks a **module-level list of forbidden words** and asks membership of
  ...
  `tests/test_release_wiring.py::_names_a_non_suppressed_credential`: it reads
  no file, compiles no pattern and parses no source, so none of the three
  rules above would see it. The rule is narrow on purpose — the iteration
  must be over a module-level *collection* constant and the body must ask an
  `in` question — because "references a constant and says `in` somewhere"
  would report ordinary table-driven tests, and a meta-scan that cries wolf
  gets an allowlist bolted to it and stops meaning anything, **or**
* it filters *the caller's* list by membership — `[t for t in terms if t in
  haystack]` — the same grep with the word list hoisted to the caller. No
  helper here has that shape today; the rule is kept, and planted, because
  the sibling suite this was ported from found a twenty-one-call-site helper
  of exactly that shape that the four rules above had cleared.
...
with no membership test and no pattern (`tests/test_vendor_pins.py`'s hash
comparison, `tests/test_seed_dirs_in_step.py`'s byte comparison — both
planted anyway), is outside the rule above. Closing those needs an
interpreter, not a reader; this scan is the AST-grep half, not a promise
...
#: Word-stems a scan helper's name tends to carry, split on "_" so
...
#: read off this repo's real test names (`..._is_caught_by_this_check`,
#: `..._divergence_is_caught`, `..._catches_...`), not guessed. The word is
...
#: prose about the thing under test — `tests/test_release_wiring.py` says
#: "no tag workflow fires, nothing publishes" about GitHub's event suppression
#: and not about a plant at all, and in the suite this was ported from the
#: same sentence was clearing a credential scan that had no plant. A word that
#: common in a repo's own subject matter cannot also be its evidence of one.
...
def _is_scan_helper(node: ast.FunctionDef, constants: frozenset[str] = frozenset()) -> bool:
    ...
    each, or filters the caller's list by membership) or its name carries one
    of the stems above.
...
#: above could not reach it — while `_global_scan_helper_names()` below would
...
#: this file exists to forbid, so a shared module is swept too, against plants
#: anywhere in `tests/`. This repo has none today; the sweep is planted below
#: against a synthetic one so the day one is added it is already covered.
...
def _offenders() -> list[str]:
    ...
    same file calls — and `tests/_scans.py` if it exists, whose helpers are
...
def test_the_meta_scan_finds_a_grep_shaped_scan(tmp_path):
    ...
    `in`. The repo has this shape already (`test_corpus_counts.py`'s
    `_count_raw_entries`), and a discovery rule that only knew about `ast`
...
def test_the_plant_check_does_not_fire_on_a_real_guarded_file():
    ...
    `tests/test_source_hosts.py` defines one scan (`_literal_hosts`, the AST
    walk over every URL literal in a source function) and plants it, so the
    meta-scan must clear it — or it would be crying wolf on the very files it
    exists to clear."""
    source = (TESTS_DIR / "test_source_hosts.py").read_text(encoding="utf-8")
    assert _scan_helpers(source) == ["_literal_hosts"], (
        "test_source_hosts.py is expected to define exactly this scan helper"
    )
    assert _unplanted_scan_helpers(source) == []


# ── the shapes a first draft cannot see ──────────────────────────────────────
...
def test_the_meta_scan_finds_a_regex_shaped_scan(tmp_path):
    ...
    (`banned_spellings`) carrying none of the stems. `tests/test_corpus_counts.py`'s
    `_corpus_readme_claims` and `_top_readme_claim` are this shape."""
    source = _write(
        tmp_path,
...
def test_the_meta_scan_finds_a_forbidden_word_list_scan(tmp_path):
    """Planted: the fourth shape, and a real one here.
    `tests/test_release_wiring.py::_names_a_non_suppressed_credential` walks
    `NON_SUPPRESSED_CREDENTIALS` asking membership of text its caller hands
    it — it reads no file, compiles no pattern, parses no source, and its
    name carries no stem, so the first three rules all clear it."""
    source = _write(
        tmp_path,
        ...
        "    assert not phrase_hits('a corpus keeps its own citations')\n",
    )
...
def test_the_word_list_rule_does_not_fire_on_ordinary_constant_use(tmp_path):
    ...
    source = _write(
        ...
        "    return rows[0]\n"
        "\n"
        "def test_every_rung_round_trips():\n"
        "    for r in RUNGS:\n"
        "        assert r in ('human', 'machine', 'asserted')\n",
    )
    assert _scan_helpers(source) == []
...
def test_the_meta_scan_finds_a_scan_handed_its_word_list_by_the_caller(tmp_path):
    ...
    *caller's* list asking membership of each — no module-level constant, no
    ...
    it. No helper here has this shape today; the rule stays because the
    suite this was ported from found one with twenty-one call sites that
...
def test_a_plant_reaches_a_shared_helper_through_an_import_alias(tmp_path):
    """The shared-module sweep is cross-module, so the reach has to follow
...
def test_the_shared_module_sweep_catches_a_planted_unplanted_helper(tmp_path, monkeypatch):
    """Planted: this repo has no `tests/_scans.py` today, so the sweep is
    proven against a synthetic tree — a shared module with two scan helpers,
    one reached by a plant test in a sibling file (through an alias) and one
    reached by nothing. Only the second may be reported, and without the
    sweep neither would be."""
    (tmp_path / "_scans.py").write_text(
        "def terms_found(haystack, terms):\n"
        "    return [t for t in terms if t in haystack]\n"
        "\n"
        "def lonely_scan(haystack, terms):\n"
    ...
    monkeypatch.setattr(this_module, "SHARED_SCANS", tmp_path / "_scans.py")

    assert _unplanted_shared_scan_helpers() == ["lonely_scan"]
    assert _offenders() == ["_scans.py: ['lonely_scan']"]
...
def test_a_docstring_saying_fires_about_something_else_does_not_clear_a_scan(tmp_path):
    """The counter-example the suite this was ported from found live: a test
    whose docstring says "no tag workflow fires" — prose about GitHub's event
    suppression, not about a plant — was clearing a credential scan that had
    no plant at all. This repo carries the same sentence in
    `tests/test_release_wiring.py`. Only "plant" counts in a docstring;
    "fires" and "catches" count in a test's name, where they are deliberate."""
    source = _write(
        tmp_path,
...
# it — and this repo writes them: `tests/test_egress.py` walks every module's
# AST for URL opens inside one test, `tests/test_sources.py` does the same for
# `requests` and for plain-http literals. This half closes that gap: it walks
# every `test_*` function's own body for the same shapes the module-helper
# half already knows, holds it to the same "reads a file, not a string it
# built" requirement, and clears a test that delegates to a scan helper
# already defined (and already required to be planted) somewhere in `tests/`
# — recognized by name, whichever module actually owns it, since
# `from tests.test_x import _helper` and a bare call to a same-file helper are
# the same delegation by a different route.
#
...
# counts; a dict key or a `Path.parents` check several steps removed does
# not. Loosening that would also catch `"x" in pyproject["project"]` and
...
def _is_inline_scan(
    ...
    **Delegation clears the whole test, deliberately.** A test that calls a
    ...
    question of the text it read, because that second question is, in
    practice, the fixture precondition of the first ("the file really does
    carry the thing I am about to plant"). The cost is real and named: an
    unplanted inline `in` hidden behind a delegated call is not seen. The
...
#: The inline scans this port found on 2026-09-12 and did not factor out:
#: each is a test whose own body reads a real file and judges it, with no
#: helper to name and so nothing to plant. A **ratchet, not an allowlist**:
#: the gate below asserts the set of inline scans is *exactly* this, so a new
#: inline scan fails it, and so does a stale entry once its test is factored
...
def test_no_test_body_is_an_unrecorded_inline_scan():
    """The inline half of the house rule, as a ratchet: a guard written
    directly in a test's body, with no helper to name and plant, can never be
    shown to fire. Every such test must be either factored into a planted
    helper or recorded in `INLINE_SCANS_STILL_TO_FACTOR` — and every entry
    there must still name a live inline scan, so the record cannot go stale."""
    offenders = frozenset(_inline_scan_offenders())
    new = sorted(offenders - INLINE_SCANS_STILL_TO_FACTOR)
    assert not new, (
        "these tests are themselves a scan (reads a real file, then walks its "
        "source and judges it, matches a pattern, or asks a membership question "
        "of the text) with no scan helper behind them anywhere in tests/, so the "
        f"scan has never been planted — factor each into a planted helper: {new}"
    )
    stale = sorted(INLINE_SCANS_STILL_TO_FACTOR - offenders)
    assert not stale, (
        "these entries no longer name an inline scan — factored out, renamed or "
        f"gone — so delete them; the ratchet only ever shrinks: {stale}"
    )
...
def test_the_inline_scan_check_finds_a_scan_written_directly_in_a_test_body(tmp_path):
    ...
    `tmp_path`, not a string built in place) — the shape of
    `tests/test_egress.py::test_no_module_opens_a_url_outside_this_one`. No
    helper exists anywhere for it to delegate to, so it must be reported."""
...
def test_the_inline_scan_check_knows_every_spelling_of_reading_a_real_file(tmp_path):
    ...
    reaches for, so the same scan written with the builtin `open`, with a
    `with` block, through `read_bytes().decode()`, or through
    `inspect.getsource` must be reported too — a boundary, not a spelling the
...
def test_the_inline_scan_check_does_not_fire_on_a_string_the_test_built(tmp_path):
    ...
    source = _write(
        ...
        "    assert not re.search(r'banned', 'a corpus keeps its own citations')\n",
    )
...
def test_a_usefixtures_marker_does_not_exempt_a_test_from_either_half(tmp_path):
    """Planted: the exemption that must not be one. `@pytest.mark.usefixtures`
    carries the word "fixture", and a rule that looked for that word anywhere
    in the decorator would clear every test wearing it. The marker must not
    exempt; a real `@pytest.fixture` beside it still must."""
    marked = _write(
        tmp_path,
...
def test_the_inline_scan_check_clears_a_test_that_delegates_to_a_known_helper(tmp_path):
    ...
    target = tmp_path / "target.py"
    target.write_text("x = 'https://example.org'\n", encoding="utf-8")
    source = _write(
        tmp_path,
        ...
        f"TARGET = Path({str(target)!r})\n"
        "\n"
        "from tests.test_source_hosts import _literal_hosts\n"
        "\n"
        "def test_the_module_names_no_undeclared_host():\n"
        "    fn = ast.parse(TARGET.read_text(encoding='utf-8')).body[0]\n"
        "    assert 'example.org' not in _literal_hosts(fn)\n",
    )
    assert _inline_scan_tests(source, frozenset({"_literal_hosts"})) == []
...
def test_the_inline_scan_check_reads_class_methods_and_async_tests_too(tmp_path):
    ...
    writes no test classes and no async tests today; the day one is written,
    a scan inside it must be as reportable as one at module level."""
    in_a_class = _write(
        tmp_path,
...
def test_delegating_clears_a_test_that_also_asks_its_own_question(tmp_path):
    ...
    keeps a plant honest ("the fixture really does carry the thing I am about
    to change"). Reporting those would push preconditions out of sight for
    no gain."""
    source = _write(
        tmp_path,
        "test_planted_delegate_and_check.py",
        "from pathlib import Path\n"
        "from tests.test_corpus_counts import _corpus_readme_claims\n"
        "\n"
        "def test_the_planted_count_is_reported():\n"
        "    text = Path('x').read_text(encoding='utf-8')\n"
        "    assert 'composing to' in text\n"
...
def test_a_local_function_sharing_a_helpers_name_does_not_clear_a_scan(tmp_path):
    ...
    imported = _write(
        ...
        "from pathlib import Path\n"
        "from tests.test_cards import _strings\n"
        "\n"
        "def test_no_banned_word():\n"
        "    text = Path('x').read_text(encoding='utf-8')\n"
        "    assert _strings(text)\n"
    ...
    assert _inline_scan_tests(imported, frozenset({"_strings"})) == [], (
...
def test_the_inline_scan_check_does_not_fire_on_the_real_tree():
    """The whole thing against a real file of this repo's:
    `tests/test_corpus_counts.py` used to run its README regexes inline in two
    tests and now delegates to `_corpus_readme_claims`/`_top_readme_claim`,
    planted in the same file — so the check must clear it, or it would be
    crying wolf on the very fix it exists to require."""
    global_helpers = _global_scan_helper_names()
    assert {"_corpus_readme_claims", "_top_readme_claim"} <= global_helpers
    source = (TESTS_DIR / "test_corpus_counts.py").read_text(encoding="utf-8")
