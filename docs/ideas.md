...
18. The fleet CI floor (fleet plan decision 5, Wave 4: C4-tests-yml, C4-codeql): a Linux test matrix derived from `pyproject.toml`'s Python classifiers rather than typed into the workflow, a Windows job at the floor and ceiling Pythons, ruff pinned to an exact current release with `ruff format --check` beside `ruff check`, CodeQL on python and actions, and an aggregate `test` gate that fails when any leg is anything but success — skipped and cancelled included — with a structural test that holds each of those in place.

19. Make `_parse_ddg_html`'s engine-link filter host-based: `jeles/reactions/search_adapter.py` drops a result when `"duckduckgo.com" in url`, a substring test CodeQL reports as incomplete URL sanitization (py/incomplete-url-substring-sanitization) and which also drops any unrelated page whose path merely mentions the engine; compare the parsed host instead. Found by the CI floor's CodeQL run on #85, where the same shape in three tests was fixed; this one is package code and its commit type would cut a release, so it waits for a `fix:` of its own.
